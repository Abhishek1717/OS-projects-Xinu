...
int	resched()
{
	...
	for( i=0;i<NFRAMES;i++){
		int store,page;
		if(frm_tab[i].fr_pid==prev )
			if(frm_tab[i].fr_type==FR_PAGE && bsm_lookup(prev,frm_tab[i].fr_vpno*NBPG,&store,&page)==OK)
				write_bs((i+FRAME0)*NBPG,store,page);
		
		if(frm_tab[i].fr_pid==currpid)
                        if(frm_tab[i].fr_type==FR_PAGE && bsm_lookup(currpid,frm_tab[i].fr_vpno*NBPG,&store,&page)==OK)
	                   read_bs((i+FRAME0)*NBPG,store,page);
