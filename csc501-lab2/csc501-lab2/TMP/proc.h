...
struct	pentry	{
	...
	int	lockhold[50];   //what locks it has to hold*************************
	int	pinh;              // priority that it has inherited
	int	lwait;		   // on which lock is it waiting
	int	queuewaittime;      //waittime in the queue	
 	int	createlockhold[50];	
};
